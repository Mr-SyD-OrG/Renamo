...
def fetch_real_name(input_string):
    """Fetch the real API."""
    url = f"http://www.omdbapi.com/?apikey=34a169c2&s={input_string}&type=series"
    response = requests.get(url).json()
    if response.get("Response") == "True":
        # Extract all series titles
        series_list = [item["Title"] for item in response.get("Search", [])]
    ...
    async with semaphore:
        try:
            ...
            if chat_d.startswith(('http')):
                ...
                await message.reply_text("⚡ chat link. ⚡")
      ...
      #  topic_syd_id = file_details['topic']
        ...
        try:  # Replace with the actual thread ID of the topic
            await client.copy_message(
                chat_id=topppsyd,  # Replace with the target group ID
                from_chat_id=mrsyd,
